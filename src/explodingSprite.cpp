...
void ExplodingSprite::makeChunks(unsigned int n)
{
  ...
  while (source_y < getFrame()->getHeight())
  {
    ...
    while (source_x < getFrame()->getWidth())
    {
      ...
      Frame* frame = new Frame(spriteSurface, chunk_width, chunk_height, source_x, source_y);
      Chunk chunk(Vector2f(X() + source_x, Y() + source_y), Vector2f(sx, sy), getName() + "/Chunk", frame);
